...
def pack_rle_record(x: int, y: int, q: int, byteorder: Literal["little", "big"] = 'little') -> bytes:
    r = (x << 20) | (y << 10) | q
    return r.to_bytes(4, byteorder=byteorder)
#
#
def unpack_rle_record(data: bytes, byteorder: Literal["little", "big"] = 'little') -> tuple[int, int, int]:
    r = int.from_bytes(data, byteorder=byteorder)
    return (r >> 20) & 0xFFF, (r >> 10) & 0x3FF, r & 0x3FF


def encode(img, threshold=127):
    ...
    for y in range(height):
        ...
        while x < width:
            ...
            if intensity <= threshold:
                ...
                buffer += pack_rle_record(start_x, y, count)
            else:
                x += 1
...
def decode(buffer: bytes, size: tuple[int, int] | list[int]) -> Image.Image:
    ...
    for n in range(0, len(buffer), 4):
        x, y, count = unpack_rle_record(buffer[n:n + 4])
...
if __name__ == '__main__':
    ...
    b1 = pack_rle_record(1, 2, 3)
    b2 = ReticleData.build(dict(x=1, y=2, q=3))
    print(b1, b2)
    c1 = unpack_rle_record(b1)
